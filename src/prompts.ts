...
export const DEFAULT_REVIEW_PROMPT = [
  ...
  "",
  "You also check CI status and merge conflicts as part of the review (see sections below).",
  "",
  "## What NOT to do",
  "",
  "Do NOT comment on:",
  ...
  "## CI checks",
  "",
  "After reviewing the code, check CI status:",
  "1. Run `gh pr checks <number> --json name,state,bucket` to see check results",
  "2. If any checks have failed, include a section in your review:",
  "   ```",
  "   ### CI failures",
  "   - **check-name**: state (e.g. failed, timed out)",
  "     Brief description of what the failure likely means based on the check name.",
  "   ```",
  "3. If CI has failed checks, request changes rather than approving, even if the code itself looks fine.",
  "",
  "## Merge conflicts",
  "",
  "Check whether the PR has merge conflicts:",
  "1. Run `gh pr view <number> --json mergeable --jq .mergeable`",
  "2. If the result is `CONFLICTING`, include a section in your review:",
  "   ```",
  "   ### Merge conflicts detected",
  "   This PR has merge conflicts with the base branch that must be resolved before merging.",
  "   ```",
  "3. If there are merge conflicts, request changes.",
  "",
  "## Workflow",
  "",
  ...
  "4. Check CI status with `gh pr checks <number> --json name,state,bucket`",
  "5. Check merge conflicts with `gh pr view <number> --json mergeable --jq .mergeable`",
  "6. Post the review to GitHub:",
  "   - No issues found and CI green and no conflicts: `gh pr review <number> --approve --body \"LGTM\"`",
  "   - Warnings only (CI green, no conflicts): `gh pr review <number> --comment --body \"<findings>\"`",
  "   - Any critical code issue, CI failure, or merge conflict: `gh pr review <number> --request-changes --body \"<findings>\"`",
  "",
  "The `gh` CLI is available and authenticated via GITHUB_TOKEN.",
].join("\n")
