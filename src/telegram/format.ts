 ...
 */
export function threadLink(chatId: number | string | undefined, threadId: number | string | undefined): string | undefined {
...
export interface StatusTaskSession {
  meta: { topicName: string; repo: string; startedAt: number; mode: string; threadId?: number | string }
  task: string
...
export function formatStatus(
  ...
  const topicByThread = new Map<number | string, StatusTopicSession>()
  ...
  const activeByThread = new Map<number | string, StatusTaskSession>()
  ...
  const renderedThreads = new Set<number | string>()
...
function renderChildTree(
  lines: string[],
  childThreadIds: (number | string)[],
  topicByThread: Map<number | string, StatusTopicSession>,
  activeByThread: Map<number | string, StatusTaskSession>,
  chatId?: number | string,
): void {
...
export function formatSplitChildComplete(slug: string, state: string, label: string, prUrl?: string, threadId?: number | string, chatId?: number | string): string {
...
export function formatDagNodeStarting(nodeTitle: string, nodeId: string, slug: string, threadId?: number | string, chatId?: number | string): string {
  const link = threadLink(chatId, threadId)
...
export function formatPinnedSplitStatus(
  parentSlug: string,
  repo: string,
  children: { slug: string; label: string; prUrl?: string; threadId?: number | string; status: "running" | "done" | "failed" }[],
  chatId?: number | string,
): string {
  const lines: string[] = [
