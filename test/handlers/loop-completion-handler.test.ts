...
import { LoopCompletionHandler, type LoopOutcomeRecorder, type LoopSchedulerProvider, type LoopThreadCleaner } from "../../src/handlers/loop-completion-handler.js"
...
function makePlatform(): ChatPlatform {
  return {
    name: "test",
    chatId: "123",
    chat: {
    ...
    threads: {
      createThread: vi.fn().mockResolvedValue({ threadId: "100", name: "test" }),
...
describe("LoopCompletionHandler", () => {
  ...
  it("skips non-loop sessions", async () => {
    ...
    const handler = new LoopCompletionHandler(makeProvider(scheduler), platform, cleaner)
    const ctx = makeCtx()
  ...
  it("skips when scheduler is null", async () => {
    const handler = new LoopCompletionHandler(makeProvider(null), platform, cleaner)
    const ctx = makeCtx({
  ...
  it("sends alert when consecutive failures reach threshold", async () => {
    ...
    const html = (platform.chat.sendMessage as ReturnType<typeof vi.fn>).mock.calls[0][0] as string
  ...
  it("handles telegram send failure gracefully", async () => {
    ...
    const scheduler = makeScheduler(states, defs)
    ;(platform.chat.sendMessage as ReturnType<typeof vi.fn>).mockRejectedValue(new Error("network error"))
    const handler = new LoopCompletionHandler(makeProvider(scheduler), platform, cleaner)
    const ctx = makeCtx({
      topicSession: makeTopicSession({ loopId: "lint-sweep" }),
      state: "errored",
  ...
  it("auto-closes the loop thread after completion", async () => {
    ...
    expect(platform.threads.deleteThread).toHaveBeenCalledWith("42")
  ...
  it("does not auto-close non-loop sessions", async () => {
    ...
    expect(platform.threads.deleteThread).not.toHaveBeenCalled()
  })
