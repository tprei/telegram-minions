...
import { makeMockChatProvider, makeMockThreadManager } from "./test-helpers.js"

vi.mock("node:fs")

function makeChat() {
  return makeMockChatProvider({
...
describe("PinnedMessageManager", () => {
  ...
  describe("updatePinnedSummary", () => {
    it("sends a new pinned summary when none exists", async () => {
      ...
      const topicSessions = new Map<string, TopicSession>()
      topicSessions.set("1", makeSession())

      const mgr = new PinnedMessageManager({ chat, threads, topicSessions, workspaceRoot: "/tmp/workspace" })
      ...
      expect(chat.pinMessage).toHaveBeenCalledWith("100")
    })
    ...
    it("edits existing pinned message when one exists", async () => {
      ...
      expect(chat.editMessage).toHaveBeenCalledWith("50", expect.stringContaining("bold-fox"))
    ...
    it("falls back to new message if edit fails", async () => {
      const chat = makeChat()
      const threads = makeThreads()
      ;(chat.editMessage as ReturnType<typeof vi.fn>).mockResolvedValue(false)
      vi.mocked(fs.readFileSync).mockReturnValue(JSON.stringify({ messageId: "50" }))
      ...
      expect(chat.editMessage).toHaveBeenCalledWith("50", expect.any(String))
    ...
    it("shows 'No active minion sessions' when empty", async () => {
      ...
      expect(chat.sendMessage).toHaveBeenCalledWith("No active minion sessions.")
    })
    ...
    it("includes header with total session count", async () => {
      ...
      const html = (chat.sendMessage as ReturnType<typeof vi.fn>).mock.calls[0][0] as string
    ...
    it("adds thread hyperlinks when chatId is provided", async () => {
      ...
      const threads = makeThreads()
      const topicSessions = new Map<string, TopicSession>()
      topicSessions.set("42", makeSession({ threadId: "42" }))

      const mgr = new PinnedMessageManager({ chat, threads, topicSessions, workspaceRoot: "/tmp/workspace", chatId: "-1001234567890" })
      mgr.updatePinnedSummary()
    ...
    it("shows correct status icons for sessions", async () => {
      ...
      const threads = makeThreads()
      const topicSessions = new Map<string, TopicSession>()
      topicSessions.set("1", makeSession({ threadId: "1", slug: "running-one" }))
      topicSessions.set("2", makeSession({ threadId: "2", slug: "done-one", activeSessionId: undefined, prUrl: "https://pr/1" }))
      topicSessions.set("3", makeSession({ threadId: "3", slug: "errored-one", activeSessionId: undefined, lastState: "errored" }))
      topicSessions.set("4", makeSession({ threadId: "4", slug: "idle-one", activeSessionId: undefined }))
    ...
    it("coerces legacy numeric messageId from persisted JSON", async () => {
      const chat = makeChat()
      const threads = makeThreads()
      vi.mocked(fs.readFileSync).mockReturnValue(JSON.stringify({ messageId: 50 }))
  ...
  describe("pinThreadMessage", () => {
    it("sends and pins a new message in a thread", async () => {
      ...
      expect(chat.sendMessage).toHaveBeenCalledWith("<b>Status</b>", "1")
      expect(chat.pinMessage).toHaveBeenCalledWith("100")
    ...
    it("edits an existing pinned message", async () => {
      ...
      expect(chat.editMessage).toHaveBeenCalledWith("42", "<b>Updated</b>", "1")
      expect(chat.sendMessage).not.toHaveBeenCalled()
    })
  })
  ...
  describe("updateTopicTitle", () => {
    it("calls editThread with emoji and slug", async () => {
      const chat = makeChat()
      const threads = makeThreads()
      const topicSessions = new Map<string, TopicSession>()
      const session = makeSession()
      ...
      expect(threads.editThread).toHaveBeenCalledWith("1", "✅ test-repo · bold-fox")
    })
  })
})
